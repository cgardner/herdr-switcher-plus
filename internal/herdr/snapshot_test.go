...
const worktreeBody = `{"id":"x","result":{"snapshot":{
  "workspaces":[
    {"workspace_id":"w5","label":"nix","number":1,
     "worktree":{"repo_name":"nix-darwin","repo_root":"/c/nix-darwin/",
                 "checkout_path":"/c/nix-darwin/","is_linked_worktree":false}},
    {"workspace_id":"w3R","label":"software-factory","number":2,
     "worktree":{"repo_name":"Second Brain","checkout_path":"/c/sf","is_linked_worktree":true}},
    {"workspace_id":"w19","label":"zmk-config","number":3}],
  "agents":[]}}}`
...
func TestParseSnapshotDecodesTheWorktree(t *testing.T) {
	...
	if w.Worktree.RepoName != "nix-darwin" || w.Worktree.IsLinked {
		t.Errorf("decoded wrong: %+v", w.Worktree)
	}
	if w.Worktree.CheckoutPath != "/c/nix-darwin/" {
