...
func TestCollectAttachesRepositoryAndBranch(t *testing.T) {
	snap := &herdr.Snapshot{
		...
		Workspaces: []herdr.Workspace{{
			WorkspaceID: "w1", Label: "software-factory", Number: 3,
			Worktree: &herdr.Worktree{
				RepoName: "Second Brain", IsLinked: true,
				CheckoutPath: "/checkouts/software-factory",
			},
		}},
	}
	var asked string
	stubSources(t, snap, nil, nil, nil)
	stubBranch(t, func(p string) string { asked = p; return "software-factory" })
	...
	r := rows[0]
	if r.Repo != "Second Brain" || !r.Linked || r.Branch != "software-factory" {
		t.Errorf("worktree not attached: %+v", r)
	}
	if asked != "/checkouts/software-factory" {
		t.Errorf("branch resolved from %q, want the checkout path", asked)
	}
	if got := r.Label(); got != "Second Brain ⑂ software-factory" {
...
func TestCollectSkipsBranchLookupWithoutAWorktree(t *testing.T) {
	snap := &herdr.Snapshot{
		Agents:     []herdr.Agent{agent("w1:p1", "w1", "")},
		Workspaces: []herdr.Workspace{{WorkspaceID: "w1", Label: "zmk-config", Number: 1}},
	}
	stubSources(t, snap, nil, nil, nil)
	...
	if got := rows[0].Label(); got != "zmk-config" {
