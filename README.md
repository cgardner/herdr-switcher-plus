...
| label | meaning |
|---|---|
| `zmk-config` | the space name tells you everything |
| `nix-darwin / nix` | an ordinary checkout in a differently named space |
| `Second Brain ⑂ software-factory` | a linked worktree of another repository |
| `scouts-scraping  @scoutbook-plus-api` | a space whose name hides its branch |
...
made half the rows repeat themselves (`scouts-scraping  scouts-scraping`) while
...
leaves them out as implied, so `/Second Brain` finds every worktree of it. Every
